...
import torchvision
def set_stochastic_depth(model, stochastic_depth_prob):
    total_stage_blocks = len( [m for m in model.modules() if isinstance(m, torchvision.models.convnext.CNBlock)] )
    stage_block_id = 0
    for stage in model.features:
        if isinstance(stage, nn.Sequential) and isinstance(stage[0], torchvision.models.convnext.CNBlock):
            for m in stage:
                if not isinstance(m, torchvision.models.convnext.CNBlock):
                    continue
                m.stochastic_depth.p = stochastic_depth_prob * stage_block_id / (total_stage_blocks-1)
                stage_block_id+=1

import timm
def set_timm_drop_path(model, drop_path_rate=0.1):
    depths = []
    for stage in model.stages:
        depths.append(len(stage.blocks))

    dp_rates = [x.tolist() for x in torch.linspace(0, drop_path_rate, sum(depths))]
    block_id = 0
    for stage in model.stages:
        for block in stage.blocks:
            block.drop_path.drop_prob = dp_rates[block_id]
            block_id+=1